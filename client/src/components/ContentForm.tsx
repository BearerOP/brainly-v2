...
const ContentForm: React.FC<ContentFormProps> = ({
  ...
  const [tags, setTags] = useState(
    initialData?.tags?.map((tag) => tag.title) || []
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (payload?.contentId && updateModal && initialData?.contentId) {
      ...
      const updatedContent = contentStore.map((content) =>
        content.contentId === enrichedContent.contentId
          ? enrichedContent
          : content
      );
      ...
      try {
        ...
        await axios.post(`${BASE_URL}/content`, enrichedContent, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } catch (error) {
        ...
        const filteredContent = contentStore.filter(
          (content) => content.contentId != enrichedContent.contentId
        );
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div
        className="bg-white p-5 rounded-lg shadow-xl relative w-[250px] md:w-[350px] text-black font-font1
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block mb-2 font-bold" htmlFor="tags">
              Tags
            </label>
            <div className="flex">
              <input
